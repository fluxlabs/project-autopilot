...
Every command should start with a clear header:
...
```
  ...
  Options:
    • /autopilot:resume --task=3.2    Resume from failed task
    • /autopilot:status               Check current state
...
─────────────────────────────────────────────────────────────
  ...
  Tip: Use /autopilot:build -y to skip approval next time.
```
