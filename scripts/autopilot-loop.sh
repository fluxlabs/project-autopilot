...
main() {
    ...
    if [[ ! -f "$PROJECT_DIR/.autopilot/checkpoint.md" ]]; then
        if [[ -z "$AUTOPILOT_PROMPT" ]]; then
            ...
            log "Either:"
            log "  1. Run /autopilot:build first to create initial checkpoint"
...
Environment Variables:
    AUTOPILOT_PROMPT    Initial prompt for /autopilot:build (required if no checkpoint)
...
The loop will:
    1. Run Claude with /autopilot:resume (or initial prompt)
